...
\begin{itemize}
\item Il prof. \textit{Andrea Maurino} per gli opportuni dettagli tecnici consigliatici, oltre che per l'opportunità di poterci cimentare con differenti problematiche necessarie per la nostra crescita professionale;
\item Il prof. \textit{Antonio Cabitza} per tutti i consigli datici per rappresentare al meglio i risultati del nostro lavoro, in modo da poterlo valorizzare maggiormente;
\item Il prof. \textit{Vincenzo Cutrona} per la costante presenza e pazienza nei momenti di bisogno;
\item Il nostro compagno, e amico, \textit{Alessandro Borroni}, per aver realizzato per noi il logo del progetto;
\item L'\textit{Università degli Studi di Milano-Bicocca} per averci fornito l'accesso alla macchine virtuali su Microsoft Azure, necessarie al supporto del nostro work project.
\item La piattaforma \textit{Meetup} per la condivisione tutt'altro che scontata dei loro dati.
\end{itemize}
