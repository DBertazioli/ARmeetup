...
\Keywords{Meetup --- ArangoDB --- SocialNetwork} % Keywords - if you don't want any simply remove all the text between the curly brackets
...
Sfruttando il particolare concetto di grafo implementato in ArangoDB abbiamo così realizzato una \textit{collection} per ogni vertice ed una \textit{edge-collection} per ogni arco del grafo.
...
Per esplorare a fondo le informazioni in nostro possesso, abbiamo sfruttato i metadati spaziali presenti nei messaggi per realizzare una visualizzazione interattiva in forma di mappa esplorabile focalizzata sull'Europa...
